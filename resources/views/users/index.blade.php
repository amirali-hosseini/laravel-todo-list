        ...
        <main class="w-full flex-grow">
            <div class="w-full p-3 mt-2">
                ...
                <div class="flex items-center justify-between mb-4">
                    <h1 class="w-full text-xl font-estedad-bold text-black pb-4">کاربران</h1>
                        ...
                        <tbody>
                            @forelse ($users as $user)
                                <tr class="hover:bg-grey-lighter border-b border-secondary-gray">
                                    ...
                                    <td
                                        class="py-4 px-6 {{ $user->hasVerifiedEmail() ? 'text-green-600' : 'text-red-600' }}">
                                        {{ $user->email_verified_at?->format('Y-m-d') ?? 'تایید نشده' }}
                                    </td>
                                    <td class="py-4 px-6 {{ $user->isAdmin() ? 'text-red-600' : 'text-green-600' }}">
                                        {{ $user->isAdmin() ? 'ادمین' : 'کاربر' }}
                                    </td>
                                    <td class="py-4 px-6">
                                        <div class="flex items-center justify-center gap-3">
                                            <a href="{{ route('users.edit', $user) }}" class="text-lg text-yellow-400">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="bg-blue-200 p-3">
                                        کاربری ای جهت نمایش وجود ندارد.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
